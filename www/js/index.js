...
function initDB(){
    var shortName = 'Cadastro';
    var version = '1.0';
    var displayName = 'Cadastro';
    var maxSize = 65536; // Em bytes
    localDB = window.openDatabase(shortName, version, displayName, maxSize);
}

function createTables(){
    var query = 'CREATE TABLE IF NOT EXISTS pessoa(id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, nome VARCHAR NOT NULL, idade VARCHAR NOT NULL);';
    try {
        localDB.transaction(function(transaction){
            transaction.executeSql(query, [], nullDataHandler, errorHandler);
            updateStatus("Tabela 'pessoa' status: OK.");
        });
    } 
    catch (e) {
        updateStatus("Erro: Data base 'cadastro' não criada " + e + ".");
        return;
    }
}
...
function onUpdate(){
    ...
    if (nome == "" || idade == "") {
        ...
        var query = "update pessoa set nome=?, idade=? where id=?;";
        try {
            localDB.transaction(function(transaction){
...
function onCreate(){
    ...
    if (nome == "" || idade == "") {
        ...
        var query = "insert into pessoa (nome, idade) VALUES (?, ?);";
        try {
            localDB.transaction(function(transaction){
